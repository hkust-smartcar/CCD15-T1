...
float ic_Kd = 0.6;      //9.1
float ic_Kp = 0;
float ic_Kp_const = 3.05;    //2.51
float moving_gain = 1.6;      //2.7
float ic_Ki = 0;
float mg = 0;
...
//float encoder_Kd = 81;
float encoder_r_Kp = 18.53;      //8.05v  //21.73       31         7.79V
float encoder_r_Kd = 0;       //2.01
float encoder_r_Ki = 0;
float encoder_l_Kp = 22.05;    //8.05v    //19.05                          28.9          7.76V
...
int main()
{
	...
	pGrapher.addWatchedVar(&ideal_count,"1");
	//	pGrapher.addWatchedVar(&ic_Kp_const,"2");
	//	pGrapher.addWatchedVar(&ic_Kd,"3");
	//	pGrapher.addWatchedVar(&moving_gain,"5");
	pGrapher.addWatchedVar(&count_l,"2");
	pGrapher.addWatchedVar(&count_r,"3");

	pGrapher.addWatchedVar(&encoder_l_Kp,"4");
	pGrapher.addWatchedVar(&encoder_r_Kp,"5");
	pGrapher.addWatchedVar(&trust_accel,"6");
	pGrapher.addWatchedVar(&original_angle,"7");
	...
	while(1){
		...
		if(t !=System::Time()){
			...
			//
			//
			//				motor_l.SetPower(abs(speed_l) + 33.62192);                 //2.4988,42
			//				motor_r.SetPower(abs(speed_r) + 50.15344);                //2.38436,72.15344
			//
			//				//				lincoln.Set(0);
			//			}
			//			//
			...
			//				//					moving_accel_flag = 0;
			//				//				}
			//				if(moving_accel_1 && moving_accel_2 && moving_accel_3 && moving_accel_4 && moving_accel_5 && moving_accel_6){  //  && moving_accel_7 && moving_accel_8
			//					accel_angle = (moving_accel_1 + moving_accel_2 + moving_accel_3 + moving_accel_4 + moving_accel_5 + moving_accel_6)/6;//  + moving_accel_7 + moving_accel_8
			//				}
			//
			...
			////					square_sign = -1;
			////				}
			//				ic_Kp = moving_gain*aabbss(output_angle - original_angle)/20 + ic_Kp_const;
			//				ideal_count = (int32_t)(original_angle + ic_Kp*(now_angle_error)+ ic_Kd*angle_error_change);
			...
			//				//				}
			//
			//				//				if(moving_ideal_count_1 && moving_ideal_count_2 && moving_ideal_count_3 && moving_ideal_count_4){
			//				//					ideal_count = (moving_ideal_count_1 + moving_ideal_count_2 + moving_ideal_count_3 + moving_ideal_count_4)/4.0f;
			//				//				}
			//				//
			//				//
			//				//
			...
			if(t % 10 ==0){
				...
				for(int i=62; i < 118; i = i+5){
					...
					if((now_5pixel_value - last_5pixel_value)/5.0f < -150*aabbss(pixel_avg_difference)){
						r_edge = i;
						r_color_flag = white_black;
						pixel_difference_sum = 0;
						break;
					...
					else if((now_5pixel_value - last_5pixel_value)/5.0f > 150*aabbss(pixel_avg_difference)){
						r_edge = i;
						r_color_flag = black_white;
						pixel_difference_sum = 0;
						break;
						...
						if((int)((pixel[65]+pixel[75]+pixel[85]+pixel[95]+pixel[110]+pixel[122])/6) > 233){
							r_color_flag = half_white;
							pixel_difference_sum = 0;
							break;
						}
						else if((int)((pixel[65]+pixel[75]+pixel[85]+pixel[95]+pixel[110]+pixel[122])/6 )< 136){
							r_color_flag = half_black;
							pixel_difference_sum = 0;
							break;
						}
				...
				for(int i = 69; i > 8; i = i-5){
					...
					if((now_5pixel_value - last_5pixel_value)/5.0f < -150*aabbss(pixel_avg_difference)){
						...
						if((int)((pixel[64]+pixel[55]+pixel[45]+pixel[35]+pixel[20]+pixel[5])/6) > 233){
							l_color_flag = half_white;
							pixel_difference_sum = 0;
							break;
						}
						else if((int)((pixel[64]+pixel[55]+pixel[45]+pixel[35]+pixel[20]+pixel[5])/6 )< 136){
							l_color_flag = half_black;
							pixel_difference_sum = 0;
							break;
						}
			...
			if(t%100 == 30){
				...
				for(int i=0;i<=128;i++){
					rect_.x = i;
					rect_.y = 160-(int)((old_pixel[i]*600)/255.0f);
					rect_.w = 1;
					rect_.h = 1;
					lcd.SetRegion(rect_);
					lcd.FillColor(BLACK);
				...
				for(int i=0;i<=128;i++){
					rect_.x = i;
					rect_.y = 160-(int)((pixel[i]*600)/255.0f);
					rect_.w = 1;
					rect_.h = 1;
					lcd.SetRegion(rect_);
					lcd.FillColor(GREEN);
