...
//float ic_Kd = 0.001;               // Recommend 0.2
...
int main()
{
	...
	printf("ic_Kd,real,2,0\n");
	printf("ideal_speed,real,3,0\n");
...
//			}
			...
			if((int32_t)(t-pt1) >= 7  && yo==0){
				...
				ideal_count = (int32_t)(ic_Kp * now_angle_error  + ic_Kd->GetReal() * angle_error_change / 0.003 - still_Ki * total_count_r);
